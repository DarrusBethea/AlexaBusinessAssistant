...
public static  int counter = 0;
    private static ExpenseListInfo sListInfo;
    private List<Expenses> mExpenses;


    private ExpenseListInfo(Context context) {
        ...
        for (int i = 0; i < InfoListView.size() - 1; i++) {
            ...
            //mExpenses
            _expenses.setmName(splitted[3]);
            _expenses.setmType(splitted[7]);
            _expenses.setmAmount(splitted[10].replaceAll(":", "").replaceAll("\\," , ""));
            _expenses.setmDate(Datesplitted[0]);
            mExpenses.add(_expenses);
        }


    }

    public static ExpenseListInfo get(Context context) {

        if (sListInfo == null) {
            sListInfo = new ExpenseListInfo(context);
        }
        return sListInfo;
    }


    public List<Expenses> getInfo() {
    ...
    public static class GetData extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                URL url = new URL(
                        "https://script.google.com/macros/s/AKfycbzRJecRXqinxLQxHRix6F3JmjHso5NyxNgXABdWrDIhwjM4UvY/exec?id=1KE91K1eYxlfSV9gHfI1LBNaCtAS_c-o8rTF92NlEWvg&sheet=expenses");
