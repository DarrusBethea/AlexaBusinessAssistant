...
class Handler {
    ...
    static async applyHandle(jovo, route, fromIntent) {
        ...
        if (route.type === jovo_core_1.EnumRequestType.AUDIOPLAYER && !_get(jovo.$handlers, route.path)) {
            // @deprecated
            // TODO: Test me
    ...
    async error(handleRequest) {
        ...
        if (_get(handleRequest.jovo.$config.handlers, jovo_core_1.EnumRequestType.ON_ERROR)) {
            const route = {
    ...
    mixin(app) {
         ...
         * @throws {JovoError}
         */
        jovo_core_1.Jovo.prototype.checkStateForInitializedComponentName = function (state) {
            if (state && this.$components) {
                ...
                if (matched.length > 0) {
                    throw new jovo_core_1.JovoError(`Can not use component names as states. Please rename the following states: ${matched.join(', ')}`, jovo_core_1.ErrorCode.ERR, 'jovo-framework', 'The used state contains at least one component name.', 'Use this.delegate() or rename the listed states to prevent interference.');
                }
            }
        };
        /**
         ...
         */
        jovo_core_1.Jovo.prototype.toStateIntent = async function (state, intent, validate = true) {
            // tslint:disable-line
            this.triggeredToIntent = true;
         ...
         */
        jovo_core_1.Jovo.prototype.delegate = function (componentName, options) {
            if (!this.$components[componentName]) {
                throw new jovo_core_1.JovoError(`Couldn\'t find component named ${componentName}`, jovo_core_1.ErrorCode.ERR, 'jovo-framework', `The component to which you want to delegate to, doesn't exist`, `Components are initialized using app.useComponents(...components)`, 'https://www.jovo.tech/docs/advanced-concepts/components');
            }
            if (!this.$session.$data[jovo_core_1.SessionConstants.COMPONENT]) {
                this.$session.$data[jovo_core_1.SessionConstants.COMPONENT] = [];
            }
            this.$session.$data[jovo_core_1.SessionConstants.COMPONENT].push([
                componentName,
                {
                    data: options.data || {},
         ...
         */
        jovo_core_1.Jovo.prototype.getActiveComponentsRootState = function () {
        ...
        jovo_core_1.Jovo.prototype.getActiveComponent = function () {
            const componentState = this.getActiveComponentsRootState();
            if (componentState) {
         ...
         */
        jovo_core_1.Jovo.prototype.sendComponentResponse = function (response) {
            const componentSessionStack = this.$session.$data[jovo_core_1.SessionConstants.COMPONENT];
        ...
        jovo_core_1.Jovo.prototype.toStatelessIntent = async function (intent) {
            ...
            if (componentState) {
                ...
                jovo_core_1.Log.verbose(` Removing state from component. ${activeComponent ? `(${activeComponent.name})` : ''}`);
